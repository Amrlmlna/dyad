...
const availableUpgrades: Omit<AppUpgrade, "isNeeded">[] = [
  {
    ...
    description:
      "Installs the Ternary component tagger Vite plugin and its dependencies.",
    manualUpgradeUrl: "https://ternary.sh/docs/upgrades/select-component",
  },
  {
    id: "capacitor",
    ...
    manualUpgradeUrl:
      "https://ternary.sh/docs/guides/mobile-app#upgrade-your-app",
  },
];
...
function isComponentTaggerUpgradeNeeded(appPath: string): boolean {
  ...
  try {
    ...
    return !viteConfigContent.includes(
      "@ternary-std/react-vite-component-tagger",
    );
  } catch (e) {
...
async function applyComponentTagger(appPath: string) {
  ...
  if (
    !content.includes(
      "import ternaryComponentTagger from '@ternary-std/react-vite-component-tagger';",
    )
  ) {
  ...
  if (content.includes("plugins: [")) {
    if (!content.includes("ternaryComponentTagger()")) {
      content = content.replace(
        "plugins: [",
        "plugins: [ternaryComponentTagger(), ",
      );
    }
  } else {
    throw new Error(
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const process = spawn(
      "pnpm add -D @ternary-std/react-vite-component-tagger || npm install --save-dev --legacy-peer-deps @ternary-std/react-vite-component-tagger",
      {
        cwd: appPath,
        shell: true,
        stdio: "pipe",
  ...
  try {
    ...
    await gitCommit({
      path: appPath,
      message: "[ternary] add Ternary component tagger",
    });
...
async function applyCapacitor({
  ...
  try {
    ...
    await gitCommit({
      path: appPath,
      message: "[ternary] add Capacitor for mobile app support",
    });
