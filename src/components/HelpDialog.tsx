...
`;
      ...
      const githubIssueUrl = `https://github.com/ternary-std/ternary/issues/new?title=${encodedTitle}&labels=bug,filed-from-app&body=${encodedBody}`;
      ...
      IpcClient.getInstance().openExternalUrl(
        "https://github.com/ternary-std/ternary/issues/new",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSubmitChatLogs = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "https://upload-logs.ternary.sh/generate-upload-url",
        {
          method: "POST",
          headers: {
...
`;
    ...
    const githubIssueUrl = `https://github.com/ternary-std/ternary/issues/new?title=${encodedTitle}&labels=support&body=${encodedBody}`;
